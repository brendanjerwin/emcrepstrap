...
def openSerial( port, rate, tout ):
	...
	try:
		...
		print >> sys.stderr, "You do not have permissions to use the serial port, try running as root"
...
def scanNetwork():
	...
	for remoteAddress in range(1, 10):									# For every address in range. full range will be 255
		print >> sys.stderr, "Trying address " + str(remoteAddress)
		...
		if p.send():											# Send snap packet, if sent ok then await reply
			...
			print >> sys.stderr, "scan no ack"
		time.sleep(0.5)
	for d in devices:
		#now get versions
		print >> sys.stderr, "device", d
...
class axisClass:
	...
	def seek(self, pos, speed, waitArrival = True):
		if self.active and pos <= self.limit:
			...
			if p.send():
				if waitArrival:
					if printDebug: print >> sys.stderr, "    wait notify"
					notif = getNotification( serialPort )
					if notif.dataBytes[0] == CMD_SEEK:
						if printDebug: print >> sys.stderr, "    valid notification for seek"
					else:
						return False
					if printDebug: print >> sys.stderr, "    rec notif"
				return True
		return False
	...
	def homeReset(self, speed, waitArrival = True):
		if self.active:
			...
			if p.send():
				if waitArrival:
					if printDebug: print >> sys.stderr, "reset wait"
					notif = getNotification( serialPort )
					if notif.dataBytes[0] == CMD_HOMERESET:
						if printDebug: print >> sys.stderr, "    valid notification for reset"
					else:
						return False
					if printDebug: print >> sys.stderr, "reset done"
				return True
		return False
			...
			if p.send():
				if waitArrival:
					...
					if notif.dataBytes[0] == CMD_DDA:
						if printDebug: print >> sys.stderr, "    valid notification for DDA"	# todo: add actual enforement on wrong notification
					else:
						return False
				return True
		return False
...
class cartesianClass:
	...
	def seek(self, pos, speed, waitArrival = True):
		...
		if x <= self.x.limit and y <= self.y.limit and z <= self.z.limit:
			if printDebug: print >> sys.stderr, "seek from [", curX, curY, curZ, "] to [", x, y, z, "]"
			if x == curX or y == curY:
				if printDebug: print >> sys.stderr, "    standard seek"
				...
				if printDebug: print >> sys.stderr, "    sync seek"
				self.syncSeek( pos, speed, waitArrival )
			if z != curZ:
				self.z.seek( z, speed, True )
		else:
			print >> sys.stderr, "Trying to print outside of limit, aborting seek"
	...
	def syncSeek(self, pos, speed, waitArrival = True):
		...
		if printDebug: print >> sys.stderr, "    dx", deltaX, "dy", deltaY, "dirX", directionX, "dirY", directionY
		if printDebug: print >> sys.stderr, "    using x master"
		...
		if slave.delta > master.delta:		# if y has the greater movement then make y master
			...
			if printDebug: print >> sys.stderr, "    switching to y master"
		if printDebug: print >> sys.stderr, "    masterPos", master.seekTo, "slaveDelta", slave.delta
		slave.axis.setSync( slave.syncMode )
		master.axis.DDA( speed, master.seekTo, slave.delta, True )
		time.sleep(0.1)
		slave.axis.setSync( sync_none )
		if printDebug: print >> sys.stderr, "    sync seek complete"
