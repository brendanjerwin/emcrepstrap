...
Rarely changed preferences are Import Coarseness, Mesh Type, Import Tiny Details, Infill Bridge Width Over Thickness & Infill in Direction
...
problem with the gap spanning algothm is that it will span gaps, even if there actually is a gap in the model.  If the import tiny details
preference is chosen, slice will import tiny details, even if they are thinner than the extrusion width, otherwise it won't.  Infill bridge width
...
def getCommonVertexIndex( edgeFirst, edgeSecond ):
	...
	print >> sys.stderr, ( "Inconsistent GNU Triangulated Surface" )
	print >> sys.stderr, ( edgeFirst )
	print >> sys.stderr, ( edgeSecond )
	return 0
...
def getLoopsFromCorrectMesh( edges, faces, vertices, z ):
	...
	for edge in remainingValues:
		if edge.faceIndexFirst == None:
			print >> sys.stderr, ( 'This should never happen, there is a hole in the triangle mesh, each edge should have two faces.' )
			print >> sys.stderr, ( edge )
			print >> sys.stderr, ( "Something will still be printed, but there is no guarantee that it will be the correct shape." )
			print >> sys.stderr, ( 'Once the gcode is saved, you should check over the layer with a z of:' )
			print >> sys.stderr, ( z )
			return None
...
def getPathIndexesAddPath( edges, faces, loops, remainingEdgeTable, vertices, z ):
	...
	if len( pathIndexes ) < 3:
		print >> sys.stderr, ( "Dangling edges, will use intersecting circles to get import layer at height " + z.toString() )
		del loops[ : ]
		return None
...
class SliceSkein:
	...
	def addGcodeFromThread( self, thread ):
		...
		if len( thread ) > 3:
			self.addLine( '(<loop> edge )' ) # Indicate that a loop is beginning.
