...
def getPacket(ser):
	...
	while 1:
		...
		if len(byte) > 0:
			...
			print >> sys.stderr, "Error: Serial timeout"		#clear buffer on timeout?		
		...
		if len(buffer) > 4:							# one packet length is recieved (HDB1?).
			...
			if len(buffer) >= expectedLength:				# check we have enough data, otherwise continue reading from serial.
				...
				if printIncomingPackets:		
					print >> sys.stderr, "###INCOMING PACKET##"
					p.printPacket()
					print >> sys.stderr, "###END INCOMING PACKET##"
...
class SNAPPacket:
	...
	def decode(self):					
		...
		if numLeftoverBytes > 0:
			print >> sys.stderr, "leftover bytes", numLeftoverBytes, self.leftoverBytes
		self.ACK, self.NAK = breakHDB2(self.HDB2)
		self.bytes = self.bytes[:6 + self.NDB]
		#print >> sys.stderr, "newb", self.bytes
		self.decoded = True
	...
	def sendBytes(self):
		if self.encoded == True:
			for d in self.bytes:
				#print >> sys.stderr, "sending", d, chr(d)
				self.serial.write(chr(d))	
		else:
			print >> sys.stderr, "Error: packet not encoded"
	...
	def send(self):
		...
		while retriesLeft > 0:				# try sending define number of times only
			...
			if printOutgoingPackets:
				...
				self.printPacket()
				print >> sys.stderr, "###END OUTGOING PACKET##"			
			...
			if ack:					
				...
				if printFailedPackets:
					print >> sys.stderr, "###FAILED OUTGOING PACKET##"
					self.decode() #remove need for this (tidy up)
					self.printPacket()
					print >> sys.stderr, "###END FAILED OUTGOING PACKET##"
			else:	
				print >> sys.stderr, "Error: ACK not recieved"
				if printFailedPackets:
					print >> sys.stderr, "###FAILED OUTGOING PACKET##"
					self.decode() #remove need for this (tidy up)
					self.printPacket()
		...
		print >> sys.stderr, "Error: Packet send FAILED (or reply)"
		return False
	...
	def printPacket(self):
		if self.decoded == True:
			print self.bytes
			print >> sys.stderr, "SNAP Packet:"
			if self.SYNC == 0x54:
				print >> sys.stderr, "...Sync OK"
			else:
				print >> sys.stderr, "...Sync Error"
			print >> sys.stderr, "...Check: ", self.check()		
			print >> sys.stderr, "...DATA", self.dataBytes
			print >> sys.stderr, "...CRC", self.CRC
			print >> sys.stderr, "...SAB", self.SAB
			print >> sys.stderr, "...DAB", self.DAB
			print >> sys.stderr, "...HDB1", self.HDB1, ":"	
			print >> sys.stderr, "...........NDB", self.NDB	
			print >> sys.stderr, "...HDB2", self.HDB2, ":"
			print >> sys.stderr, "...........ACK", self.ACK
			print >> sys.stderr, "...........NAK", self.NAK
			print >> sys.stderr, "END OF PACKET"
		else:
			print >> sys.stderr, "Error: packet not decoded"	
...
def makeHDB2(ACK, NAK):
	...
	#print >> sys.stderr, "HDB2 = '" + str(HDB2val) + "'"
	return HDB2val
...
def makeHDB1(NDB):
	...
	#print >> sys.stderr, "HDB1 = '" + str(HDB1val) + "'"
	return HDB1val
